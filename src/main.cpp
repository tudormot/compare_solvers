...
int main(int argc, char *argv[]) {
	...
	MPI_Init(NULL, NULL);
	ierr = MPI_Comm_size(MPI_COMM_WORLD, &no_of_nodes);
	CHKERRQ(ierr);
	ierr = MPI_Comm_rank(MPI_COMM_WORLD, &node_rank);
	CHKERRQ(ierr);
	...
	if (std::string(argv[1]) == "--solve-with-pardiso") {
		if (node_rank == 0) {
			if (no_of_nodes != 1) {
				std::cout
						<< "Error: pardiso is meant to be called with no_of_nodes = 1 \n";
				throw;
			} else {
		...
		if (node_rank == 0) {
			...
			//will store the solution in the lin_sys object, which is then compared against the petsc solution in the petsc routines
			pard_solve.solve_sys(lin_sys);
		}
		...
		lin_sys.release_mem_mat();
		petsc_solve.solve_sys(lin_sys);
		//petsc_solve.print_sol_to_file(lin_sys,out_filename); //uncomment this if you want solution to be printed to file

	} else if (std::string(argv[1]) == "--create-structsym-mat") {
		/*
		 * This option is used to create a structurally symmetric matrx from a purely symmetrical one
		 * It was written as I required a smaller structurally symmetric matrix than what
		 * was provided by MTU for quick testing
		 * */
		if (no_of_nodes != 1) {
			std::cout
					<< "Error: create-structsym-mat is meant to be called with no_of_nodes = 1 \n";
			throw;
		} else {
		...
		/*
		 * This option should be used only to debug the creation of structurally symmetric matrix process
		 * */
		if (no_of_nodes != 1) {
			std::cout
					<< "Error: compare-matrices is meant to be called with no_of_nodes = 1 \n";
			throw;
		} else {
