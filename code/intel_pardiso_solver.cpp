...
//TODO: would be nice if lin_sys_solver functions would  be part of a separate sys_mat.c file
                                    ...
                                    std::vector<int> &col_i_new,           std::vector<int> &row_ch_new,      std::vector<double> &elem_new,const bool is_asym)          //vectors storing output{
{
    ...
    if(is_asym == true)
    {
		...
		/*we assume that non_diag_old contains lower triangular stored column by column,
		 *  followed by upper triangular stored
		 * row by row*/


		/*we need a helper vector that initially specifies at which indexes of elem_new
		 * the row changes. */
		...
		for(int i = 0; i< diag_old.size()-1; i++)
		{
			...
			for (int j = row_index_start; j < row_index_end; j++)
			{
				//now store indices of elems in line i+1, on the right of the diagonal, also store elems
				col_i_new[helper_vect[i]] = row_i_old[j];
				elem_new[helper_vect[i]]  = non_diag_old [j+non_diag_old.size()/2];
				helper_vect[i]++;
				//now store indices of elems in col i+1, underneath the diagonal, also store elems elems
    ...
    else
    {
        ...
        //using the fact that the lower triangle in CCS is given, which is equal to upper triangle in CRS
...
int pardiso_solver::solve_sys(linear_sys &sys)
{
    ...
    if(sys.sol.size() != 0)
    {
        ...
        //in this case we calculate the relative error between solution provided and calculated solution
...
FAILED:
    ...
    MKL_Free_Buffers ();
    #endif
    std::cout<<"Stub function, not implemented\n";
    return true;
}
