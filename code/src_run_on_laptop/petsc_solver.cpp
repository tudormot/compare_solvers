...
PETSc_solver::PETSc_solver(int& main_argc, char**& main_argv, linear_sys& input_sys)
{
    ...
    ierr = PetscViewerASCIIOpen(PETSC_COMM_WORLD, &monitor_file[0], &viewer);
    CHKERRCONTINUE(ierr);
    ...
    //KSPMonitorDefault(ksp,PetscInt n,PetscReal rnorm,PetscViewerAndFormat *dummy);
    CHKERRCONTINUE(ierr);
...
PetscErrorCode PETSc_solver::mat_preallocate_mem(linear_sys& sys)
{
    if(sys.no_of_nodes != 1)
    {
        ...
        if(sys.node_rank == 0)
        {
            ...
            //memset(preallocation_data_SeqSBAIJ,0,sizeof(PetscInt)*sys.mat_dim);
    ...
    else
    {
        ...
        {
            preallocation_data_SeqAIJ[i]= sys.col_ch[i+1]-sys.col_ch[i];
        }
        MatSeqAIJSetPreallocation(A,PETSC_DEFAULT,preallocation_data_SeqAIJ);
    }
    CHKERRQ(ierr);
