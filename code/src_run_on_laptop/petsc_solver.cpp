...
PetscErrorCode PETSc_solver::mat_preallocate_mem_SEQAIJ(linear_sys& sys)
{
    ...
    {
        preallocation_data_SeqAIJ[i]= preallocation_data_SeqAIJ[i] + sys.col_ch[i+1]-sys.col_ch[i]+1; //+1 due to the diagonal elements
    ...
    ierr = MatSeqAIJSetPreallocation(A,PETSC_DEFAULT,preallocation_data_SeqAIJ);
    CHKERRQ(ierr);
    return ierr;
...
{
	//TODO
	std::cout<<"ERROR, MPISBAIJ not implemented yet, terminating program\n";
	throw;
	...
	//old function starts here
	{
		...
		if(sys.node_rank == 0)
		{
			...
			preallocation_data_MPIAIJ[sys.mat_dim-1][0]++; //last entry of diagonal
		  //  preallocation_data_SeqSBAIJ[sys.mat_dim-1]++;
	#ifdef MPISBAIJ_IMPLEMENTED
			...
			//also set seqaij here in case we are running the project on seq matrix

			//MatSeqAIJSetPreallocation(A,PETSC_DEFAULT,preallocation_data_SeqAIJ);
			//and for seqsbaij:
		  //  MatSeqSBAIJSetPreallocation(A,1,PETSC_DEFAULT,preallocation_data_SeqSBAIJ);
		}
		else
		{
			MPI_Gather(&nlocal,1,MPIU_INT,NULL,0,MPIU_INT,0,PETSC_COMM_WORLD);
	    ...
	    CHKERRQ(ierr);
	    //TODO: this should be removed once everything is running smoothly
